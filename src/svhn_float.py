...
class SVHNConvFloat(nn.Module):
    def __init__(self, discrt_lvls=2.):
        super(SVHNConvFloat, self).__init__()
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch > 10:
            if (args.save_model):
                torch.save(model.state_dict(), f"../model/SVHN_float.pt")

            d = [train_accuracy, test_accuracy]
            export_data = zip_longest(*d, fillvalue='')
            with open(f'../model/SVHN_float_report.csv', 'w', encoding="ISO-8859-1", newline='') as report_file:
                wr = csv.writer(report_file)
                wr.writerow(("Train accuracy", "Test accuracy"))
                wr.writerows(export_data)
            report_file.close()

    model.load_state_dict(torch.load(f"../model/SVHN_float.pt"))
