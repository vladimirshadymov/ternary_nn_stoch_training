...
from layers import TernarizedLinear, Ternarization, TernarizedConv2d
...
class SVHNConv3NN(nn.Module):
    def __init__(self, discrt_lvls=2.):
        super(SVHNConv3NN, self).__init__()
        self.pool = nn.MaxPool2d(2)
        self.dp = 0.2
        self.threshold_factor = 0.7
        self.ternary_factor = 1

        self.layer128_1 = nn.Sequential(
            TernarizedConv2d(in_channels=3, out_channels=128, kernel_size=3, stride=1, padding=1, scale_factor=0.074*self.ternary_factor*2.12, threshold=self.threshold_factor*0.074),
            nn.BatchNorm2d(128),
            nn.ReLU(),
        ...
        self.layer128_2 = nn.Sequential(
            TernarizedConv2d(in_channels=128, out_channels=128, kernel_size=3, stride=1, padding=1, scale_factor=0.012*self.ternary_factor*2.17, threshold=self.threshold_factor*0.012),
            nn.BatchNorm2d(128),
            nn.ReLU(),
            nn.MaxPool2d(2),
            # nn.Dropout2d(self.dp),
        ...
        self.layer256_1 = nn.Sequential(
            TernarizedConv2d(in_channels=128, out_channels=256, kernel_size=3, stride=1, padding=1, scale_factor=0.013*self.ternary_factor*1.84, threshold=self.threshold_factor*0.013),
            nn.BatchNorm2d(256),
            nn.ReLU(),
            # nn.Dropout2d(self.dp),
        )
        ...
        self.layer256_2 = nn.Sequential(
            TernarizedConv2d(in_channels=256, out_channels=256, kernel_size=3, stride=1, padding=1, scale_factor=0.014*self.ternary_factor*1.66, threshold=self.threshold_factor*0.014),
            nn.BatchNorm2d(256),
            nn.ReLU(),
            nn.MaxPool2d(2),
            # nn.Dropout2d(self.dp),
        ...
        self.layer512_1 = nn.Sequential(
            TernarizedConv2d(in_channels=256, out_channels=512, kernel_size=3, stride=1, padding=1, scale_factor=0.016*self.ternary_factor*1.55, threshold=self.threshold_factor*0.016),
            nn.BatchNorm2d(512),
            nn.ReLU(),
            # nn.Dropout2d(self.dp),
        )
        ...
        self.layer512_2 = nn.Sequential(
            TernarizedConv2d(in_channels=512, out_channels=512, kernel_size=3, stride=1, padding=1, scale_factor=0.013*self.ternary_factor*1.57, threshold=self.threshold_factor*0.013),
            nn.BatchNorm2d(512),
            nn.ReLU(),
            nn.MaxPool2d(2),
            # nn.Dropout2d(self.dp),
        ...
        self.fc_layer1 = nn.Sequential(
            TernarizedLinear(in_features=512 * 4 * 4, out_features=1024, scale_factor=0.0051*self.ternary_factor*1.727, threshold=self.threshold_factor*0.0051),
            nn.BatchNorm1d(1024),
            nn.ReLU(),
            # nn.Dropout2d(self.dp),
        )
        ...
        self.fc_layer2 = nn.Sequential(
            TernarizedLinear(in_features=1024, out_features=10, scale_factor=0.09*self.ternary_factor*1.48, threshold=self.threshold_factor*0.09),
        )
    ...
    def quantize_accumulative_weigths(self):
        for layer in self.modules():
            if isinstance(layer, TernarizedLinear):
                layer.quantize_accumulative_weigths()
            if isinstance(layer, TernarizedConv2d):
                layer.quantize_accumulative_weigths()
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch > 10:
            if (args.save_model):
                torch.save(model.state_dict(), f"../model/svhn_3nn.pt")

            d = [train_accuracy, test_accuracy]
            export_data = zip_longest(*d, fillvalue='')
            with open(f'../model/svhn_3nn_report.csv', 'w', encoding="ISO-8859-1", newline='') as report_file:
    ...
    model = SVHNConv3NN().to(device)
    model.load_state_dict(torch.load(f"../model/svhn_3nn.pt"))
    ...
    for i in range(input_noise_list.size):
        ...
        for j in range(weight_noise_list.size):
            ...
            pd.DataFrame(data=accuracy_test, index=input_noise_list, columns=weight_noise_list).to_csv('svhn_3nn_noise_test.csv')
