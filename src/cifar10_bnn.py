...
from layers import BinarizedLinear, BinarizedConv2d, Binarization
...
class Cifar10ConvBNN(nn.Module):
    def __init__(self):
        ...
        self.layer128_1 = nn.Sequential(
            BinarizedConv2d(in_channels=3, out_channels=128, kernel_size=3, stride=1, padding=1, scale_factor=0.1),
            nn.BatchNorm2d(128),
        ...
        self.layer128_2 = nn.Sequential(
            BinarizedConv2d(in_channels=128, out_channels=128, kernel_size=3, stride=1, padding=1, scale_factor=0.02),
            nn.BatchNorm2d(128),
            nn.ReLU(),
            nn.Dropout2d(self.dp),
            nn.MaxPool2d(2),
        ...
        self.layer256_1 = nn.Sequential(
            BinarizedConv2d(in_channels=128, out_channels=256, kernel_size=3, stride=1, padding=1, scale_factor=0.024),
            nn.BatchNorm2d(256),
            nn.Dropout2d(self.dp),
            nn.ReLU()
        )
        ...
        self.layer256_2 = nn.Sequential(
            BinarizedConv2d(in_channels=256, out_channels=256, kernel_size=3, stride=1, padding=1, scale_factor=0.022),
            nn.BatchNorm2d(256),
            nn.ReLU(),
            nn.Dropout2d(self.dp),
            nn.MaxPool2d(2),
        ...
        self.layer512_1 = nn.Sequential(
            BinarizedConv2d(in_channels=256, out_channels=512, kernel_size=3, stride=1, padding=1, scale_factor=0.02),
            nn.BatchNorm2d(512),
            nn.Dropout2d(self.dp),
            nn.ReLU()
        )
        ...
        self.layer512_2 = nn.Sequential(
            BinarizedConv2d(in_channels=512, out_channels=512, kernel_size=3, stride=1, padding=1, scale_factor=0.015),
            nn.BatchNorm2d(512),
            nn.ReLU(),
            nn.Dropout2d(self.dp),
            nn.MaxPool2d(2),
        ...
        self.fc_layer1 = nn.Sequential(
            BinarizedLinear(in_features=512 * 4 * 4, out_features=1024, scale_factor=0.007),
            nn.BatchNorm1d(1024),
            nn.Dropout(self.dp),
            nn.ReLU()
        )
        ...
        self.fc_layer2 = nn.Sequential(
            BinarizedLinear(in_features=1024, out_features=10, scale_factor=0.1),
        )
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch > 10:
            if (args.save_model):
                torch.save(model.state_dict(), f"../model/cifar10_bnn.pt")

            d = [train_accuracy, test_accuracy]
            export_data = zip_longest(*d, fillvalue='')
            with open(f'../model/cifar10_bnn_report.csv', 'w', encoding="ISO-8859-1", newline='') as report_file:
    ...
    model = Cifar10ConvBNN().to(device)
    model.load_state_dict(torch.load(f"../model/cifar10_bnn.pt"))
    ...
    for i in range(input_noise_list.size):
        ...
        for j in range(weight_noise_list.size):
            ...
            pd.DataFrame(data=accuracy_test, index=input_noise_list, columns=weight_noise_list).to_csv('cifar10_bnn_noise_test.csv')
