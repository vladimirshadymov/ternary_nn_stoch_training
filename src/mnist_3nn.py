...
from layers import TernarizedLinear, Ternarization
...
class MnistDense3NN(nn.Module):
    def __init__(self, hidden_layer_size=150):
        ...
        self.ternary_factor = 1.8

        self.layer1 = nn.Sequential(
            TernarizedLinear(in_features=28*28, out_features=hidden_layer_size, scale_factor=0.02*self.ternary_factor, threshold=self.threshold_factor*0.02),
            nn.BatchNorm1d(hidden_layer_size),
            nn.ReLU()
        )

        self.layer2 = nn.Sequential(
            TernarizedLinear(in_features=hidden_layer_size, out_features=hidden_layer_size, scale_factor=0.028*self.ternary_factor, threshold=self.threshold_factor*0.028),
            nn.BatchNorm1d(hidden_layer_size),
            nn.ReLU()
        )

        self.layer3 = nn.Sequential(
            TernarizedLinear(in_features=hidden_layer_size, out_features=10, scale_factor=0.12*self.ternary_factor, threshold=self.threshold_factor*0.12),
        )
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch>=10:
            if (args.save_model):
                torch.save(model.state_dict(), f"../model/mnist_3nn_{HIDDEN_SIZE}.pt")
            d = [train_accuracy, test_accuracy]
            export_data = zip_longest(*d, fillvalue='')
            with open(f'../model/mnist_3nn_report_{HIDDEN_SIZE}.csv', 'w', encoding="ISO-8859-1", newline='') as report_file:
