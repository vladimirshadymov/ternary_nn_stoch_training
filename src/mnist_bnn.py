...
from layers import BinarizedLinear, Binarization
...
class MnistDenseBNN(nn.Module):
    def __init__(self, hidden_layer_size=150):
        ...
        self.layer1 = nn.Sequential(
            BinarizedLinear(in_features=28*28, out_features=hidden_layer_size, scale_factor=0.02),
            nn.BatchNorm1d(hidden_layer_size),
            nn.ReLU()
        )

        self.layer2 = nn.Sequential(
            BinarizedLinear(in_features=hidden_layer_size, out_features=hidden_layer_size, scale_factor=0.028),
            nn.BatchNorm1d(hidden_layer_size),
            nn.ReLU()
        )

        self.layer3 = nn.Sequential(
            BinarizedLinear(in_features=hidden_layer_size, out_features=10, scale_factor=0.12),
        )
...
def main():
    ...
    model = MnistDenseBNN(hidden_layer_size=HIDDEN_SIZE).to(device)
